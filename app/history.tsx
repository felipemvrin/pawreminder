...
function PetPicker({ pets }: { pets: Pet[] | undefined }) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background, padding: spacing[6], gap: spacing[3] }}>
      <Text style={{ ...typography.display, color: colors.primary }}>Historial</Text>
      <Text style={{ ...typography.body, color: colors.muted }}>Selecciona una mascota para ver su historial.</Text>
      <FlatList
        data={pets}
...
function LogRow({ log, productLabel }: { log: TreatmentLog; productLabel: string }) {
  return (
    ...
    >
      ...
      {log.notes ? <Text style={{ ...typography.caption, color: colors.muted }}>{log.notes}</Text> : null}
    </View>
  );
}
...
export default function HistoryScreen() {
  ...
  const getProductLabel = (log: TreatmentLog) => {
    ...
    return treatment.productName || (treatment.type === 'internal' ? 'Tratamiento interno' : 'Tratamiento externo');
  };
